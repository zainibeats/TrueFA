...
copy /Y build\TrueFA-Setup-1.0.2.exe release\
copy /Y build\TrueFA-Portable-1.0.2.exe release\
...
cd release
certutil -hashfile TrueFA-Setup-1.0.2.exe SHA256 > TrueFA-Setup-1.0.2.exe.sha256
certutil -hashfile TrueFA-Portable-1.0.2.exe SHA256 > TrueFA-Portable-1.0.2.exe.sha256

:: Sign the executables
echo Signing executables with GPG key %KEY_ID%...
gpg --batch --yes --default-key %KEY_ID% --detach-sign TrueFA-Setup-1.0.2.exe
gpg --batch --yes --default-key %KEY_ID% --detach-sign TrueFA-Portable-1.0.2.exe
...
echo    gpg --verify TrueFA-Setup-1.0.2.exe.sig TrueFA-Setup-1.0.2.exe >> VERIFY.txt
echo    gpg --verify TrueFA-Portable-1.0.2.exe.sig TrueFA-Portable-1.0.2.exe >> VERIFY.txt
echo. >> VERIFY.txt
echo 2. To verify the SHA256 checksum: >> VERIFY.txt
echo    certutil -hashfile TrueFA-Setup-1.0.2.exe SHA256 >> VERIFY.txt
echo    Compare the output with the contents of TrueFA-Setup-1.0.2.exe.sha256 >> VERIFY.txt
echo. >> VERIFY.txt
...
cd ..
powershell -Command "Compress-Archive -Path 'release\*' -DestinationPath 'TrueFA-1.0.2-Release.zip' -Force"

echo.
echo ===== Release package created successfully! =====
echo Package location: %CD%\TrueFA-1.0.2-Release.zip
echo.
